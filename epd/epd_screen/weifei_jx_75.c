...
static void Ultrachip_new(file_id_t fd)				 //black
{
  UINT16 i,j,k;
  ...
  for( i=384; i >0 && i<=384; i--)
  {
    ...
    for(k=0;k<4;k++)
    {
      ...
      {
        data[j] &= (data_red[j]); 
        EPD_W21_WriteDATA_58(data[j]);
//        EPD_W21_WriteDATA_58(0X0F);       
      }
    }
  }
...
static void EPD_W21_Init(void){
  ...
  EPD_W21_WriteCMD(0x06);         //booster 设置
...
//////  EPD_W21_WriteDATA(0x00);
  ...
  EPD_W21_WriteDATA(0x1E);		//decide by LUT file
  
  EPD_W21_WriteCMD(0xe5);			//FLASH MODE		   	
  EPD_W21_WriteDATA(0x03);	
  EPD_W21_WriteCMD(0x10);	       //传送图像
...
//  EPD_W21_WriteDATA(0x01);
//  
//  EPD_W21_CS1_0;					//MFCSB 拉低
//  MCU_write_flash(0xB9);
//  EPD_W21_CS1_1;					//MFCSB 拉高
//  
//  EPD_W21_WriteCMD(0X65);			//FLASH CONTROL
//  EPD_W21_WriteDATA(0x00);
//  /**********************************flash sleep**********************************/	
