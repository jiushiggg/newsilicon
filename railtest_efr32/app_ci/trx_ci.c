...
void sleep(int argc, char **argv)
{
  ...
  if (emMode <= 4)
  {
    ...
    if (emMode == 4)
    {
      switch (argv[1][1])
      {
        case 's': case 'S': // Configure EM4 Shutoff state
          ATOMIC( EMU->EM4CTRL = (EMU->EM4CTRL & ~EMU_EM4CTRL_EM4STATE)
    ...
    // the event occurring before we try to sleep.  Don't use INT_Disable()
    // here because its ref-counting gets in the way if looping below.
    __disable_irq();
    ...
    do // Loop modally here until either RfSense or Serial event occurs
    {
     ...
     #endif//DEBUG_SLEEP_LOOP
      __enable_irq(); // Briefly enable IRQs to let them run
      __disable_irq(); // but shut back off in case we loop
      rfSensed = RAIL_RfSensed();
    } while (!rfSensed && !serEvent);
    __enable_irq(); // Back on permanently
