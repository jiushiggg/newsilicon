...
void gpioCallback(uint8_t pin)
{
  ...
  if (pin == buttonArray[0].pin)
  {
    ...
    {
      // Negative Edge
      gpio0TimeCapture = halCommonGetInt32uMillisecondTick();
    }
    else
    {
      // Positive Edge
      if (elapsedTimeInt32u(gpio0TimeCapture, halCommonGetInt32uMillisecondTick()) >
          BUTTON_HOLD_MS)
      {
        gpio0LongPress();
      }
  ...
  else if (pin == buttonArray[1].pin)
  {
    ...
    {
      // Negative Edge
      gpio1TimeCapture = halCommonGetInt32uMillisecondTick();
    }
    else
    {
      // Positive Edge
      if (elapsedTimeInt32u(gpio1TimeCapture, halCommonGetInt32uMillisecondTick()) >
          BUTTON_HOLD_MS)
      {
        gpio1LongPress();
      }
